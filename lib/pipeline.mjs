...
//
// 相比初版的三点改进：
//   1. 落盘改为 debounce 合并写，不再每次 set 都同步 writeFileSync 全量重写
//      （结果对象可能很大，同步写会阻塞事件循环）；
//   2. 真正的 LRU：命中时把条目移到队尾，而非只按插入顺序淘汰；
//   3. 增加 TTL，过期条目在加载与读取时剔除，避免 /tmp 缓存只增不减。
function hashStr(s) {
  let h = 5381;
...
const CACHE_MAX = Number(process.env.LW_CACHE_MAX) || 50;
const CACHE_TTL_MS = Number(process.env.LW_CACHE_TTL_MS) || 7 * 24 * 60 * 60 * 1000; // 默认 7 天
const CACHE_PERSIST_DEBOUNCE_MS = 2000;

// key → { ts, value }
const _resultCache = new Map();
const _inflight = new Map(); // 进行中的计算，用于并发去重（相同文本只算一次）
...
try {
  ...
  if (obj && typeof obj === 'object') {
    ...
    for (const [k, v] of Object.entries(obj)) {
      // 兼容旧格式（直接存结果，无 ts 包装）
      const entry = (v && typeof v === 'object' && 'ts' in v && 'value' in v)
        ? v
        : { ts: now, value: v };
      if (now - entry.ts > CACHE_TTL_MS) continue;
      _resultCache.set(k, entry);
      restored++;
    }
    if (restored) console.log(`[Cache] 已从磁盘恢复 ${restored} 条缓存`);
  }
} catch { /* 首次运行无缓存文件，忽略 */ }
...
function persistCacheNow() {
  try {
    fs.writeFileSync(CACHE_FILE, JSON.stringify(Object.fromEntries(_resultCache)));
  } catch (e) {
    console.warn('[Cache] 落盘失败（忽略）:', e.message);
  }
}
...
function cacheSet(key, val) {
  ...
  _resultCache.set(key, { ts: Date.now(), value: val });
...
// 长文自动分片（每片约 < CHUNK_CHARS，避免超上下文被截断/超 token）；单批失败仅该片回退规则引擎。
// 返回 { enhanced, llmUsed, usage } —— llmUsed 表示是否真正成功调用了 LLM（用于真实上报），
// usage 为真实 token 用量（OpenAI 兼容格式，多片累加），未调用则为 null。

// 对一批章节做单次 LLM 调用。失败抛错，由上层决定回退；成功返回 { enhanced, usage }。
async function enhanceBatch(sections) {
  ...
  const enhanced = sections.map((s, i) => {
    const e = arr[i];
    if (!e) return s;
...
async function enhanceWithLLM(structured) {
  ...
  // 长文分片：每片累计正文约 < CHUNK_CHARS，避免超上下文被截断/超 token
  const CHUNK_CHARS = 6000;
  const batches = [];
  let cur = [], n = 0;
  for (const s of structured) {
    const len = (s.content || []).join('\n').length;
    if (cur.length && n + len > CHUNK_CHARS) { batches.push(cur); cur = []; n = 0; }
    cur.push(s); n += len;
  }
  ...
  for (let bi = 0; bi < batches.length; bi++) {
    try {
      const r = await enhanceBatch(batches[bi]);
      enhanced.push(...r.enhanced);
      llmUsed = true;
      usage = usage ? mergeUsage(usage, r.usage) : r.usage;
    } catch (e) {
      console.warn(`[LLM] 第 ${bi + 1}/${batches.length} 片归纳失败，该片回退规则引擎:`, e.message);
      enhanced.push(...batches[bi]); // 该片保持原样
    }
  }
  return { enhanced, llmUsed, usage };
}
...
// 子串包含判断是 O(n²)，长文的 n-gram 候选可达数千个，全量参与会明显卡顿。
// 最终只取 topN（5~8）个词，先按频次截断到此上限，
// 对结果的影响可忽略，但把比较次数从百万级降到十万级。
const KEYWORD_CANDIDATE_LIMIT = 400;
...
// 口语填充词。预编译为常量正则：原先每次请求都重新 new RegExp，
// 对长文 + 高频调用是纯浪费（顺序保持不变，行为与原来一致）。
const FILLER_WORDS = [
  '那个','嗯','啊','就是','然后','对吧','怎么说呢','其实吧',
  '就是说','然后呢','对对对','是吧','嘛','哈','哈哈','呃',
  'like','you know','I mean','sort of'
];
const FILLER_RES = FILLER_WORDS.map(w => new RegExp(w, 'g'));
...
// 两节之间至少要共享多少个概念词才认为存在关联
const RELATION_MIN_SHARED_TERMS = 2;

// 先在章节级聚合 token，再做章节两两比较。
// 原先按「要点」两两比较，复杂度 O((章节数 × 每节要点数)²)；
// 聚合后降为 O(章节数²)，长文（要点数十上百）下差异明显。
// 由于 i < j 天然保证每对章节只出现一次，无需再去重。
function detectRelations(structured) {
  const sectionGrams = structured.map(s => tokenize(s.keyPoints.join('\n')));
  const raw = [];
  for (let i = 0; i < sectionGrams.length; i++) {
    for (let j = i + 1; j < sectionGrams.length; j++) {
      const overlap = [...sectionGrams[i]].filter(g => sectionGrams[j].has(g));
      if (overlap.length >= RELATION_MIN_SHARED_TERMS) {
        raw.push({ from: i, to: j, sharedTerms: overlap.slice(0, 5) });
      }
    }
  }
  return raw;
...
export async function processTranscript(rawText, { onEvent, useCache = true } = {}) {
  ...
  if (cacheKey) {
    ...
    if (hit) {
      // 不直接改缓存中的对象，避免把 cached 标记写回磁盘缓存
      const result = { ...hit, stats: { ...hit.stats, cached: true } };
      emit('status', { stage: 'cache', message: '命中缓存，秒回结果' });
      emit('done', { result, llmUsed: hit.stats.llmUsed, cached: true });
      return result;
    }
  }

  // ─── In-flight 去重：相同文本正在计算时，复用同一 Promise，避免重复花 token ───
  if (cacheKey && _inflight.has(cacheKey)) {
    emit('status', { stage: 'dedup', message: '已有相同请求在处理，合并等待…' });
    // 首个请求失败时，等待方也必须收到 error 事件；
    // 否则 await 直接抛出，前端 SSE 流会没有收尾事件。
    let result;
    try {
      result = await _inflight.get(cacheKey);
    } catch (e) {
      emit('error', { message: e.message });
      throw e;
    }
    emit('done', { result, llmUsed: result.stats.llmUsed, cached: false });
    return result;
  }
  ...
  const p = run();
  if (cacheKey) _inflight.set(cacheKey, p);
  try {
...
// 以下纯函数一并导出，供 tests/ 下的单元测试直接验证（不影响运行时行为）
export { preprocess, detectRelations, segmentTopics, buildSection, tokenize, cleanSentence };
